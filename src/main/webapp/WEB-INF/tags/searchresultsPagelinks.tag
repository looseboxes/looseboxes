...
<%--@debug p> Pages per batch: ${sr_pagesPerBatch}, Pages remainder: ${sr_pagesRem}, Pages begin: ${sr_pagesBegin}, Target end: ${sr_targetEnd}, Pages end: ${sr_pagesEnd}</p --%>    
    ...
    <div class="myEm1 fullWidth borderless searchMessage">
      ...
      <c:forEach varStatus="vs" begin="${sr_pagesBegin}" end="${sr_pagesEnd - 1}">
        <c:choose>
          <c:when test="${vs.index == searchResultsBean.pageNumber}">
            <c:set var="sr_pagelinkClass" value="srPageLinksSelected"/>
          </c:when>
          <c:otherwise>
            <c:set var="sr_pagelinkClass" value="srPageLinks"/>  
          </c:otherwise>
        </c:choose>    
        <a class="${sr_pagelinkClass}" href="${searchServletPath}?page=${vs.index}">${vs.index + 1}</a>&nbsp;&nbsp;
      </c:forEach>   
